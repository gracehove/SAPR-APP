...
struct PathView: View {
    ...
    private func run(){
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            ...
            let sms = "sms: \(passedContact.phoneNumber)&body=I'm walking back to \(passedLocation.description) with the Path Deviation function. My current location is \(passedLocation.coordinate))"
            ...
            while(MKMapPoint(manager.center).distance(to: MKMapPoint(passedLocation.coordinate)) > 30){
                ...
                if((abs(startingRadius - currentRadius)) < 15){
                    //Inform user that you are ending the expect me function:
                    let sms = "sms: \(passedContact.phoneNumber)&body=I'm walking back to \(passedLocation.description) with the Path Deviation function and seem to have stopped. My current location is \(passedLocation.coordinate))"
                    let strURL : String = sms.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
                    UIApplication.shared.open(URL.init(string: strURL)!, options: [:], completionHandler: nil)
                    
                } else if ( currentRadius > (startingRadius + 20)){ //Going the wrong way?
                    //Inform user that you are ending the expect me function:
                    let sms = "sms: \(passedContact.phoneNumber)&body=I'm walking back to \(passedLocation.description) with the Path Deviation function and seem to be going in the wrong direction. My current location is \(passedLocation.coordinate))"
                    let strURL : String = sms.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
                    UIApplication.shared.open(URL.init(string: strURL)!, options: [:], completionHandler: nil)
                }
                //Update these guys:
